define(['../../app', '../../common/utilities'], function(app, UTILITIES, moment) {
	'use strict';
	return ['$scope', '$state', '$http', 'CONSTANT', '$mdDialog', '$mdToast', function($scope, $state, $http, CONSTANT, $mdDialog, $mdToast) {
		...
		// 删除
		$scope.delMsg = function(ev, msgId) {
			ev.stopPropagation();
			...
			$mdDialog.show(confirm).then(function() {
				$http.post(CONSTANT.SERVICE.MAIL.DELETE + '?msgId=' + msgId).success(function(data) {
					if (data.code == 0) {
						$scope.loadList();
					}
					$mdToast.show(
